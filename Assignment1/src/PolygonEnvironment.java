...
public class PolygonEnvironment extends JPanel {
    ...
    public void paintComponent(Graphics polygon) {
        ...
        // setting all nodes' total scores and startToNode cost
        for (int i = 0; i < allNodes.length; i++) {
            allNodes[i].setTotalScore(pathCost(startNode, allNodes[i]) + allNodes[i].getEstimatedNodeCost());
            allNodes[i].setStartToNode(pathCost(startNode, allNodes[i]));
        }

//===============================================A* IMPLEMENTATION START===============================================
        // implementing the A* algorithm
        PriorityQueue<VerticesNodes> unvisitedNodes = new PriorityQueue<>(compareNodes);
        PriorityQueue<VerticesNodes> visitedNodes = new PriorityQueue<>(compareNodes);
        ArrayList<VerticesNodes> solutionPathNodes = new ArrayList<>();
        ArrayList<VerticesNodes> currentNeighbors;
        VerticesNodes currentNode;
        double currentCost;
        ...
        while (unvisitedNodes.size() != 0) {
            ...
            // if the current node is the goal node, implementation ends; adding the goal node to solution list if not added already
            if (currentNode == goalNode) {
                if (!solutionPathNodes.contains(goalNode)) {
                    solutionPathNodes.add(goalNode);
                }
                break; // all solution nodes should be added to the solutionPathNodes array at this point
            }
            ...
            for (int i = 0; i < currentNeighbors.size(); i++) {
                // calculating the current cost of the current node by adding current node's distance and the path from current node to its neighbors
                currentCost = currentNode.getStartToNode() + pathCost(currentNode, currentNeighbors.get(i));

                // if the neighbor isn't visited and is greater than the current cost, then pursue this path
                if (unvisitedNodes.contains(currentNeighbors.get(i))) {
                    if (currentNeighbors.get(i).getStartToNode() >= currentCost) {
                        continue;
                    }

                // if the neighbor is visited and is greater than the current cost, remove from visited and pursue path
                } else if (visitedNodes.contains(currentNeighbors.get(i))) {
                    if (currentNeighbors.get(i).getStartToNode() >= currentCost) {
                        visitedNodes.remove(currentNeighbors.get(i));
                        unvisitedNodes.add(currentNeighbors.get(i));
                        continue;
                    }

                // if the neighbor is in neither visited or unvisited, then add to the unvisited queue
                } else {
                    unvisitedNodes.add(currentNeighbors.get(i));
                }
                // set the current neighbor's total score and its distance from start
                currentNeighbors.get(i).setStartToNode(currentCost);
                currentNeighbors.get(i).setTotalScore(currentNeighbors.get(i).getStartToNode() + currentNeighbors.get(i).getEstimatedNodeCost());
            }

            // establish current node and add to the solutions nodes array
            visitedNodes.add(currentNode);
            solutionPathNodes.add(currentNode);
        }
//================================================A* IMPLEMENTATION END================================================
    ...
    Comparator<VerticesNodes> compareNodes = new Comparator<VerticesNodes>() {
        ...
        public int compare(VerticesNodes o1, VerticesNodes o2) {
            if ((o1.getEstimatedNodeCost() + o1.getTotalScore()) < (o2.getEstimatedNodeCost() + o2.getTotalScore())) {
                return -1;
            } else {
                return 1;
            }
