...
public class NoObstacles2 extends JPanel {
    @Override
    public void paintComponent(Graphics polygon) {
        ...
        int goal2X[] = {100, 100, 150, 150};
        int goal2Y[] = {55, 105, 105, 55};
        Polygon goalArea2 = new Polygon(goal2X, goal2Y, 4);
        shapes.fillPolygon(goalArea2);

        int goal3X[] = {700, 700, 750, 750};
        int goal3Y[] = {550, 600, 600, 550};
        Polygon goalArea3 = new Polygon(goal3X, goal3Y, 4);
        shapes.fillPolygon(goalArea3);

        // start point with node
        VerticesNodes startNode = new VerticesNodes(750, 350, 0, 0);
        shapes.setColor(Color.red);
        shapes.fillOval(750, 350, 20, 20);

        int startX[] = {700, 700, 800, 800};
        int startY[] = {300, 400, 400, 300};
...
//===============================================RRT IMPLEMENTATION START===============================================
        ...
        for (int i = 0; i < newNode.length; i++) {
            if (goalArea.contains(currentNode.getxValue(), currentNode.getyValue()) || goalArea2.contains(currentNode.getxValue(), currentNode.getyValue()) ||
                    goalArea3.contains(currentNode.getxValue(), currentNode.getyValue())) {
                break;
            }
